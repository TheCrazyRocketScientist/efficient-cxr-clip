...
class HuggingfaceImageEncoder(nn.Module):
    def __init__(
        ...
        if hasattr(self.image_encoder.config, "hidden_size"):
            self.out_dim = self.image_encoder.config.hidden_size
        elif hasattr(self.image_encoder.config, "num_features"):
            self.out_dim = self.image_encoder.config.num_features
        elif hasattr(self.image_encoder, "num_features"):
            self.out_dim = self.image_encoder.num_features
        else:
            self.out_dim = 448 
    ...
    def forward(self, image):
        ...
        elif self.model_type == "fastvit":
            return self.image_encoder(pixel_values=image)
