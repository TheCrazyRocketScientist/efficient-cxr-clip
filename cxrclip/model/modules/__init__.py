...
def load_text_encoder(config_text_encoder: Dict, vocab_size: int):
    ...
    revision = config_text_encoder.get("revision", "main")
    fd = config_text_encoder.get("force_download", False)
    cache_dir = config_text_encoder["cache_dir"]
    lf = os.path.exists(os.path.join(cache_dir, f'models--{config_text_encoder["name"].replace("/", "--")}'))
