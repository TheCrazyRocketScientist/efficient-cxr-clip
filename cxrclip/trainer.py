...
def run(local_rank, cfg: Dict):
    ...
    if util.GlobalEnv.get().master:
        #wandb init and patching into tensorboard under master process  
        wandb.init(
            project="CXR-CLIP",
            name=f"{cfg.model.image_encoder.name.split('/')[-1]}_{cfg.model.text_encoder.name.split('/')[-1]}",
            config=OmegaConf.to_container(cfg, resolve=True),
            sync_tensorboard=True, # This tells WandB to look for TB files
            job_type="pretrain",
            group="H100-April"
        )
    ...
    if util.GlobalEnv.get().master:
        # Get names for model
        v_name = cfg.model.image_encoder.name.split('/')[-1]
        t_name = cfg.model.text_encoder.name.split('/')[-1]
        
        log.info(f"Uploading best model artifact: CLIP_{v_name}_{t_name}")
        
        # Define the artifact
        artifact = wandb.Artifact(
            name=f"CLIP_{v_name}_{t_name}", 
            type="model",
            metadata={"best_val_loss": best_loss}
        )
        
        # Point to the file created during the loop
        best_model_path = os.path.join(cfg["base"]["output"]["checkpoint"], "model-best.tar")
        
        if os.path.exists(best_model_path):
            artifact.add_file(best_model_path)
            wandb.log_artifact(artifact)
        else:
            log.warning("Artifact file not found for upload!")
